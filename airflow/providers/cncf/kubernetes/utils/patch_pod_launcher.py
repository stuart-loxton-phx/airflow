...
class PodLauncher(LoggingMixin):
    ...
    def read_pod_logs(self, pod, tail_lines=10):
        ...
        try:
            ...
            def log_reader(name, namespace):
                log_line_offset = 0

                while True:
                    try:
                        reader = self._client.read_namespaced_pod_log(
                            name = name,
                            namespace = namespace,
                            container = 'base',
                            follow = True,
                            _request_timeout = (10, 1800), # 10s connect timeout, 30m read timeout
                            _preload_content = False
                        )

                        # Read to log_line_offset. Used when the loop re-executes due
                        # to timeouts and retry
                        offset = 0
                        for line in reader:
                            offset += 1
                            if offset <= log_line_offset:
                                continue
                        ...
                        self.log.info("Timeout on k8s log reply, reconnecting at offset %i" % log_line_offset)
                        continue

            return log_reader(pod.metadata.name, pod.metadata.namespace)
